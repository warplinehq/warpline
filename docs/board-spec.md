...
| `plugin_refused` | an advance did not fire a plugin that holds a content approval, either because the approval stopped applying (`metadata_json.reason`: `indeterminate`, `outside_window`, `content_moved`) or because the spend mark's own state-document I/O failed, so nothing was sent (`mark_unavailable`, `mark_uncertain`) |
